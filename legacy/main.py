...
def getColor(R, G, B):
    ...
    for i in range(len(dataset)):
        dist = (
            abs(R - int(dataset.loc[i, "R"]))
            + abs(G - int(dataset.loc[i, "G"]))
            + abs(B - int(dataset.loc[i, "B"]))
        )
        if dist <= minimum:
            minimum = dist
            color_found = dataset.loc[
                i, "color_name"
            ]  # finding the color that corresponds to the found distance
